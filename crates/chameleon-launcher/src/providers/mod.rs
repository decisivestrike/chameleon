...
use gtk::{
    ...
    ListScrollFlags, ListView, SingleSelection, SortListModel, SorterChange,
    gio,
};
    ...
    D: ItemData,
{
    ...
    pub fn view(&self) -> gtk::ListBase {
        match self.view.clone() {
            View::List(list_view) => list_view.upcast(),
            View::Grid(grid_view) => grid_view.upcast(),
        }
    }
    ...
    pub fn reset_selection(&self) {
        if self.len() > 0 {
            ...
            match &self.view {
                View::List(list_view) => {
                    list_view.scroll_to(0, ListScrollFlags::SELECT, None)
                }
                View::Grid(grid_view) => {
                    grid_view.scroll_to(0, ListScrollFlags::SELECT, None)
                }
            };
        }
    }
