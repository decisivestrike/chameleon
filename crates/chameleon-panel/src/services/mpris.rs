//! This module was stolen from [Ironbar](https://github.com/JakeStanger/ironbar)
    ...
    pub(crate) fn new() -> Self {
...

        {
            ...
            RUNTIME.spawn_blocking(move || {
                let player_finder =
                    PlayerFinder::new().expect("Failed to connect to D-Bus");

                // D-Bus gives no event for new players,
                // so we have to keep polling the player list
                loop {
                    // mpris-rs does not filter NoActivePlayer errors, so we
                    // have to do it ourselves
                    let players =
                        player_finder.find_all().unwrap_or_else(|e| match e {
                            mpris::FindingError::DBusError(
                                DBusError::TransportError(transport_error),
                            ) if transport_error.name()
                                == Some(NO_ACTIVE_PLAYER)
                            ...
                            _ => {
                                error!(
                                    "D-Bus error getting MPRIS players: {e:?}"
                                );
                                vec![]
                            }
                        });

                    // Acquire the lock of current_player before players to
                    // avoid deadlock. There are places
                    // where we lock on current_player and players, but we
                    // always lock on current_player first.
                    // This is because we almost never need to lock on players
                    // without locking on current_player.
                    {
                        let mut current_player_lock =
                            current_player.lock().unwrap();

                        let mut players_list_val = players_list.lock().unwrap();
...

        {
            ...
            RUNTIME.spawn_blocking(move || {
                let player_finder =
                    PlayerFinder::new().expect("to get new player finder");

                loop {
                    Self::send_tick_update(
                        &player_finder,
                        &current_player,
                        &tx,
                    );
        ...
        RUNTIME.spawn_blocking(move || {
            ...
            if let Ok(player) = player_finder.find_by_name(&player_id) {
                ...
                >| {
                    ...
                    // Lock of player before players (see new() to make sure
                    // order is consistent)
                ...
                for event in player.events()? {
                    trace!(
                        "Received player event from '{identity}': {event:?}"
                    );
                    match event {
                        ...
                        Err(mpris::EventError::DBusError(
                            DBusError::TransportError(transport_error),
                        )) if transport_error.name()
                            == Some(NO_ACTIVE_PLAYER)
                            || transport_error.name() == Some(NO_REPLY)
                        ...
                        Ok(_) => {
                            ...
                            if let Some(current_identity) =
                                current_player_lock.as_ref()
                                && current_identity == identity
                                && let Err(err) =
                                    Self::send_update(&player, &tx)
                            {
                                if let Some(DBusError::TransportError(
                                    transport_error,
                                )) = err.downcast_ref::<DBusError>()
                                    && transport_error.name()
                                        == Some(NO_SERVICE)
                                {
...
impl MusicClient for Client {
    ...
    fn seek(&self, duration: Duration) -> Result<()> {
        if let Some(player) = Self::get_player(self) {
            // if possible, use `set_position` instead of `seek` because some
            // players have issues with seeking see https://github.com/JakeStanger/ironbar/issues/970
    ...
    fn subscribe_change(&self) -> broadcast::Receiver<PlayerUpdate> {
        ...
        if let Some(player) = self.get_player() {
            ...
            self.tx
                .send(PlayerUpdate::Update(Box::new(None), status))
                .unwrap();
        }
...
pub enum PlayerUpdate {
    ...
    /// such as a new track playing, the player being paused, or a volume
    /// change.
